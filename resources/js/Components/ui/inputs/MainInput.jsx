...
export default function MainInput({ label, required = false, type = "text", value, onChange, error, placeholder = "", className = "" }) {
  const inputClasses = `mt-3.5 bg-border rounded-lg text-text-medium text-xs border font-normal py-3 leading-none px-3.5 outline-none ${className} ${error ? 'border-red-300' : 'border-input-border'}`;
